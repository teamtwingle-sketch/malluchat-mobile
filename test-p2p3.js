...
(async () => {
  ...
  console.log("USER 2 CLICKS USER1 NAME to start private chat...");
  await page2.evaluate(() => {
      window.confirm = () => true;
  ...
  console.log("USER 1 CHECKS FOR TOAST & ACCEPTS...");
  await page1.evaluate(() => {
      const acceptBtn = [...document.querySelectorAll('button')].find(b => b.innerText === 'Accept');
  ...
  await page2.evaluate(() => {
      ...
      const sendBtn = [...document.querySelectorAll('button')].find(b => b.innerHTML.includes('Send')); // SVG might be hard to match
  });
  // press enter to send instead
