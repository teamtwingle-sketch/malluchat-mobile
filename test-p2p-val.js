...
(async () => {
  ...
  // Send public message from P1
  await page1.type('input[placeholder="Send to public..."]', 'ping');
  await page1.keyboard.press('Enter');
  
  await new Promise(r => setTimeout(r, 2000));
  
  // P2 clicks User1 chat
  console.log("P2 requests chat...");
  ...
  // P2 sends MSG
  console.log("P2 sending secure message...");
  await page2.type('input[placeholder="Type a secure message..."]', 'secret123');
  await page2.keyboard.press('Enter');
  
  await new Promise(r => setTimeout(r, 2000));

  const p1Html = await page1.content();
  console.log("USER 1 GOT SECRET MESSAGE?", p1Html.includes('secret123'));
